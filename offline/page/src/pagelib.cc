...
void PageLib::create() {
    DirScanner dir;
    dir(_config->get_configs().find("XMLDIR")->second); //逐个读文件并存入files<vector>
    for (string & filename : dir.files()) {
        ...
        while(itemNode) {
            ...
            if(itemNode->FirstChildElement("title")) {
                ...
                title = regex_replace(title, reg, ""); //标签替换为空字符串
...
void PageLib::Page_De_Duplication() {
    ...
    for(size_t i = 0; i < _rss.size(); ++i) {
        uint64_t u = 0; //64位simhash值
        simhasher.make(_rss[i]._description, topN, u); //提取关键词的 Top-N 权重，生成 64 位 SimHash 值
        Sim.push_back(std::make_pair(i, u)); //存储网页索引i 和 simhash值u
    }
    set<int> tmp; //存储重复网页索引
...
void PageLib::handle_Invert_Index()
{
    ...
    // hello (1, 3)  (2, 10) (3, 7)
    unordered_map<int, double> docW;  //存储每个文档中所有词权重的平方和
    for(auto it = tf.begin(); it != tf.end(); ++it) {
        string word = it->first;       //hello
        size_t DF = it->second.size(); //DF该词出现的文档数，
        for(auto num = it->second.begin(); num != it->second.end(); ++num){
            int docid = num->first;
            int TF = num->second;
            double IDF = log(1.0 * _DOCICNUM /(DF + 1))/log(2); //以2为底
            double w = TF * IDF;
            num->second = w; //已经得到 hello (1, w)  (2, w) (3, w)
            docW[docid] += w * w;  //累加每个文档中所有词的 w² 到 docW
        }
    }
    ...
    for(auto it = tf.begin(); it != tf.end(); ++ it) {
        ...
        for(auto elem = it->second.begin(); elem != it->second.end(); ++elem) {
            ...
            _invertIndex[word].insert(std::make_pair(docid, FinW)); //单词 docid 权重
        }
    }
}
...
void PageLib::store() {
    ...
    for(size_t i = 0; i != _rss.size(); ++i) {
        size_t idx = i + 1;
        ofs1 << idx << " ";
        size_t  beginpos = ofs.tellp(); //获取在网页库的起始位置
        ofs1 << beginpos << " ";
        ofs << "<doc>"<<endl;
        ...
        ofs << "</doc>" <<endl;
        size_t endpos = ofs.tellp();  //获取当前网页内容的结束位置
...
#if 0
void PageLib::handle_wordMap() {  //记录每个词出现的频率
...
void PageLib::handle_topK() {
    ...
    simhash::Simhasher simhasher(DICT_PATH, HMM_PATH, IDF_PATH, STOP_WORD_PATH);
    for(auto it = _docid.begin(); it != _docid.end(); ++it) {
        vector<pair<string, double>> results;  //<词，权重>
        simhasher.extract(it->second, results, topN);
        for(auto & it2 : results) {
            _topK[it->first].push_back(it2.first);  //关键词存入
        }
    }
}
#endif
